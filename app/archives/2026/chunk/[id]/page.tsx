...
// 139 lines / 7 lines per chunk = 20 chunks
const LINES_PER_CHUNK = 7

async function getChunkContent(chunkId: number) {
  const filePath = path.join(process.cwd(), "public", "archives", "2026", "all_text.jsonl")
  try {
    ...
    <div style={{ maxWidth: 1000, margin: "0 auto", padding: 24 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
        ...
        <h1 style={{ fontSize: 18, fontWeight: 600, letterSpacing: 0.2, margin: 0 }}>
          Symbi Archives 2026 — Chunk {chunkId}/{data.totalChunks}
        </h1>
      </div>

      <div style={{ fontSize: 12, opacity: 0.75, margin: "6px 0 14px" }}>
        Lines {data.range} of {data.totalLines}. 139 conversations across 7 platforms. Rendered inline for LLM accessibility.
      </div>

      <div style={{ display: "flex", gap: 10, marginBottom: 12, flexWrap: "wrap" }}>
        <a href="/archives/index.html">Back to index</a>
        {chunkId > 1 && <a href={`/archives/2026/chunk/${chunkId - 1}`}>Previous Chunk</a>}
        {chunkId < data.totalChunks && <a href={`/archives/2026/chunk/${chunkId + 1}`}>Next Chunk</a>}
        <a href="/archives/2026/combined">Full Combined</a>
      ...
      <div style={{ marginTop: 12, fontSize: 12, opacity: 0.6 }}>
        <a href="/archives/index.html">← Archives index</a>
      </div>
    </div>
  )
}
